...
export const hasUrl = (
  item: LinkItemType
): item is Extract<LinkItemType, { url: string }> =>
      ...
      <nav className="mx-auto flex h-14 w-full max-w-[1400px] items-center gap-2 px-4">
          ...
          {links.map((item, i) => (
            <li key={i}>
              <Link
                ...
                data-active={
                  item.url === activeUrl || isLinkItemActive(item, pathname)
                }
        ...
        <div className="ms-auto flex items-center gap-1.5 lg:gap-2">
          ...
          {slots.searchTrigger && slots.themeSwitch && (
            <span
              aria-hidden="true"
              className="h-4 w-px shrink-0 bg-fd-border max-lg:hidden"
            />
          )}
          {slots.themeSwitch && (
            <slots.themeSwitch
          ...
          {icons.map((item, i) => (
            <LinkItem
              ...
              item={item}
              aria-label={
                item.url === GITHUB_URL && stars !== undefined
