---
title: Marker
description: Place markers on the map with custom views, callouts, and drag support.
---
...
| `offset` | [`Point`](../types.mdx#point) | `{x: 0, y: 0}` | Pixel offset for non-bubbled callouts from their default centered position above the marker. Only applies when `bubbled` is `false` |
